...
class CPOTrainer(Trainer):
    ...
    def cpo_loss(
        ...
        def word_level_sigmoid_dot_similarity(logits1, logits2):
            """
            Computes sigmoid(dot product) similarity for each word position.

            Args:
                logits1, logits2: Tensors of shape [batch_size, sequence_length, vocab_size]

            Returns:
                Tensor of shape [batch_size, sequence_length]
            """
            batch_size, sequence_length, vocab_size = logits1.shape
            similarities = torch.zeros((batch_size, sequence_length), device=logits1.device)

            for i in range(sequence_length):
                vec1 = logits1[:, i, :]  # shape: [batch_size, vocab_size]
                vec2 = logits2[:, i, :]  # shape: [batch_size, vocab_size]

                dot_product = torch.sum(vec1 * vec2, dim=1)  # shape: [batch_size]
                sim = torch.sigmoid(dot_product)            # shape: [batch_size]
        ...
        logits = (policy_chosen_logps - (1- penalty_ndp).clamp(0.0, 1.0).mean(dim=1) * policy_rejected_logps).to(self.accelerator.device)
        ## old = cos_sim
