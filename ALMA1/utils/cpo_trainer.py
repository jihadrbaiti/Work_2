...
class CPOTrainer(Trainer):
    ...
    def concatenated_inputs(
        ...
        #print('batch["rejected_labels"].shape: ',batch["rejected_labels"].shape)
        #print('batch["chosen_labels"].shape: ',batch["chosen_labels"].shape)
        for k in batch:
        ...
        #print('concatenated_batch concatenated_labels: ', concatenated_batch['concatenated_labels'].shape)
        #print('concatenated_batch concatenated_input_ids: ', concatenated_batch['concatenated_input_ids'].shape)
        #print('concatenated_batch concatenated_attention_mask: ', concatenated_batch['concatenated_attention_mask'].shape)
    ...
    def cpo_loss(
        ...
        """
        device1 = self.device if hasattr(self, 'device') else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        def word_level_cosine_similarity(logits1, logits2):
            """
                ...
                logits2: PyTorch tensor of shape [batch_size, sequence_length, vocab_size].

            Returns:
                PyTorch tensor of shape [batch_size, sequence_length] containing the cosine similarity
                for each word position in each sentence.
            """
            batch_size, sequence_length, vocab_size = logits1.shape
            cosine_similarities = torch.zeros((batch_size, sequence_length), device=logits1.device)

            for i in range(sequence_length):
                # Get logits for the current word position across the batch
                word_logits1 = logits1[:, i, :]  # Shape: [batch_size, vocab_size]
                word_logits2 = logits2[:, i, :]  # Shape: [batch_size, vocab_size]

                # Normalize the logit vectors
                normalized_logits1 = F.normalize(word_logits1, p=2, dim=1)
                normalized_logits2 = F.normalize(word_logits2, p=2, dim=1)

                # Calculate cosine similarity for the current word position across the batch
    ...
    def concatenated_forward(
        ...
        #print('concatenated_batch["concatenated_input_ids"]',concatenated_batch["concatenated_input_ids"])
        outputs = model(
        ...
        if self.cpo_alpha == 0:
            nll_loss = torch.tensor(0.0).to(self.accelerator.device) ## nll_loss instead
        else:
        ...
        #print('num_non_pad_tokens = (concatenated_batch["concatenated_labels"] != self.label_pad_token_id): ', concatenated_batch["concatenated_labels"] != self.label_pad_token_id)
    ...
    def get_batch_loss_metrics(
        ...
        #    self.kl_penalty_beta *= 1.5
        #elif kl.item() < target_kl / 1.5:
        #    self.kl_penalty_beta /= 1.5
        
        kl_penalty_beta = max(min(kl_penalty_beta, 5.0), 1e-3)
        # end added

        alpha_bc = max(0.01 * (1 - self.state.global_step / 500), 0.0)  # from 0 to 0.01 (for nll_loss)
        alpha_bc1 = 0.1 #1 default 0.1
        # self.cpo_alpha
        loss = losses.mean() + alpha_bc1 * kl + self.cpo_alpha * policy_nll_loss # previous is alpha_bc1 * kl instead
        #print('losss',loss)
